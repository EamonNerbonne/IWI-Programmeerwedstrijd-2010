...
class State {
	...
	public long minRoute() {
		...
		int nextpos = nextIrregularity(position);
		while (nextpos != -1) {
			...
			if (dif > 0) {
				...
				if ((allneeded > 0) && (load >= allneeded)) {
					...
					// here we go back.
					int aa = position-firstIrregularity();
					if (aa <= 0) { System.out.println("ERROR: aa smallar than or equal to zero"); }
					...
					long mrb = s.minRoute();
					//System.out.println(toString());
					//System.out.println("aa = "+aa+", bb = "+bb+", mrb = "+mrb);
					//System.out.println("(backtrack)aa+mrb = "+(aa+mrb)+", (goforward)2*bb = "+(2*bb));
					// go forward if a > b then calc forward, otherwise calc back
					// where b is distance to last.
					
					// minfullroute = min(minfullroute,s.minRoute());
					// now go back
					// if bb is zero then don't calc backtracking
					if (aa+mrb < 2*bb) {
						//State s = new State(this);
						//minfullroute = min(minfullroute,s.minRoute());
			...
			nextpos = nextIrregularity(position+1);
		}
		
		// here we assume you already returned,
		// so it is smooth sailing until first irregularity;
		if (nextIrregularity(start) != -1) { // cannot be position
			if (nextIrregularity(start) >= position) { System.out.println("ERROR: nextIrregularity is greater than or equal to position"); }
